import React, { Component } from 'react';
import './ProfileEdit.css';
import axios from 'axios';
import { connect } from 'react-redux'


class ProfileEdit extends Component {

    render() {
        return (
            <div className="profilepage">
           <div className = "profile_navbar">
               <div className="helo">Helo
                   <img className="homeimage" src="data:image/png;base64,"/>
                    <img className="searchimage" src="data:image/png;base64,"/>
                   </div>
            
               <div className="profile">Profile</div>
               <div className="logout">Logout</div>

           </div>
           <div className="parentbox">
           <div className = "profilebox">
               <img className="profileimage"src="https://robohash.org/me"/>
               <button className="updatebutton">Update</button>
               <button className="searchbutton">Search</button>
           </div>
           <div className = "profile_info"></div>
           </div>
          
            </div> 
        )
    }
}

function mapStateToProps(state){
    console.log(state)
    return {

    }
}

export default connect(mapStateToProps, {} )(ProfileEdit);